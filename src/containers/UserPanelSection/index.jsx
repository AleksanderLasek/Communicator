...
const UserPanelSection = ({user}) => {
  ...
  const handleNameChange = async (e) => {
    ...
    const token = cookie.refreshToken;
    try {
        const res = await axios.post('http://localhost:5000/users/edit', {refreshToken: token, name: name, surname: surname});
        console.log(res);
        setEditUser(false);
    }catch(err){
        console.log(err);
  ...
  return (
    <S.Wrapper>
      ...
      <S.ImageWrapper>
        ...
        {avatar && ( 
          <S.Button onClick={handleAvatarChange}>Submit</S.Button>
        )}
      </S.ImageWrapper>
        <S.Text onClick={handleEditUser}>Edit name</S.Text>
        <S.InputWrapper editUser={editUser}>
          <S.Input placeholder="Enter new name" name="name" onChange={(e) => setName(e.target.value)}/>
